...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Edit Form
                        ...
                        <div>
                            <div v-for="(dynamic,index) in dynamicForm" :key="index" class="row form-group">
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" v-model="dynamic.field_label" required>
                                </div>
                                <div class="col-md-3">
                                    <label>Field Type</label>
                                    <select class="form-control" v-model="dynamic.option_type_id" @change.prevent="changeFieldType(dynamic.option_type_id,index)" required>
                                        <option v-for="types in optionTypes" :key="types.id" :value="types.id">{{types.name}}</option>
                                    </select>
                                </div>
                                <div class="col-md-3" v-if="dynamic.option_type_id == 3 ">
                                    ...
                                    <span v-for="(options,ind) in dynamic.options" :key="ind">
                                        <div class="row">
                                            <div class="col-md-8">
                                                <input type="text" class="form-control m-1" v-model="options.options" required>
                                            </div>
                                            <div class="col-md-4">
                                                <button class="btn btn-sm btn-danger p-2" @click.prevent="deleteOptionValues(index,ind,options.id)">Delete</button>
                                            </div>
                                        </div>
                                    </span>
                                </div>
                                <div class="col-md-2 p-4">
                                    <label>&nbsp;</label>
                                    <button class="btn btn-danger" @click.prevent="deleteFields(index,dynamic.id)">Delete</button>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        deleteFields(index, deletedId) {
            if (deletedId !== null) {
                this.fieldDeletedId.push(deletedId)
            }
        ...
        submitform() {
            ...
            axios.post(`/update-form`, {
                ...
                optionDeletedId: this.optionDeletedId,
                fieldDeletedId: this.fieldDeletedId
            }).then(response => {
                console.log(response.data.data);
                this.success = "Your form updated successfully"
                this.error = null
                //this.dynamicForm = []
            }).catch(err => {
        ...
        deleteOptionValues(index, ind, deletedId) {
            if (deletedId !== null) {
                this.optionDeletedId.push(deletedId)
            }
            this.dynamicForm[index].options.splice(ind, 1)
        },
        getFormData() {
            axios.get(`/get-form-data/${this.id}`).then(response => {
                console.log(response.data.data);
        ...
        changeFieldType(optionId, index) {
            if (optionId != 3) {
                this.dynamicForm[index].options = [];
            }
        }
    }
}
